...
Whenever a change is completed and verified, perform the following steps to bump the version and publish the updated package(s) to npm:

1. **Determine Version Bump (Semantic Versioning)**:
   - `patch`: Bug fixes, refactors, internal improvements without API changes.
   - `minor`: New backwards-compatible features or API additions.
   - `major`: Breaking changes requiring consumer code updates.

2. **Monorepo / Changeset Workflow** (if project uses `@changesets/cli` or `pnpm`):
   - **Automated (CI/CD)**: Create changeset (`pnpm changeset`), commit `.changeset/`, and push to `main`. GitHub Actions (`.github/workflows/release.yml`) handles package versioning, changelog updates, tagging, and npm publishing automatically via `NPM_TOKEN`.
   - **Manual Execution**:
     ```bash
     # Create a changeset entry describing the change
     pnpm changeset  # or npx changeset

     # Apply version bumps to package.json files and generate/update CHANGELOG.md
     ...
     # Build, test, and publish to npm registry
     pnpm build
     pnpm test
     pnpm changeset publish  # or pnpm -r publish --access public
     ```

3. **Single Package Workflow** (standard npm project):
   ```bash
   # Bump version in package.json & create git tag
   npm version patch  # or minor / major

   # Update CHANGELOG.md with release notes & commit
   git add .
...
5. **NPM Publishing Rules**:
   - Always run clean build (`pnpm build` / `npm run build`) and tests (`pnpm test` / `npm test`) **before** publishing.
   - Ensure you are authenticated with npm (`npm whoami`).
   - Scoped packages (`@scope/pkg`) require `--access public` unless explicitly private.
   - Never skip tag generation or changelog documentation when publishing a new version.
