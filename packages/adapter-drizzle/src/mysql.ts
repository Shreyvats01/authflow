...
export function createMysqlAdapter(db: any): BolkAuthAdapter {
  return {
    ...
    async updateSession(id: string, data: Partial<Session>): Promise<Session> {
      ...
      const session = await this.findSessionByToken(data.token ?? id);
      return session!;
    },
    ...
    async createVerificationToken(data: Omit<VerificationToken, "createdAt">): Promise<VerificationToken> {
      const now = new Date();
